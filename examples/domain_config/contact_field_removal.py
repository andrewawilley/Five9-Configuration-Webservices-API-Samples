...
if __name__ == "__main__":
    ...
    # get the list of fields to remove from the argument "fields_to_remove"
    parser = argparse.ArgumentParser(
        description="Remove specific contact fields from Five9"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="List of contact fields to remove",
    )
        ...
        if len(campaigns_in_stopping) > 0:
            ...
            with tqdm.tqdm(
                ...
                while len(campaigns_in_stopping) > 0:
                    ...
                    pbar.set_postfix({"Campaigns still STOPPING": campaigns_in_stopping_still_count})
                    pbar.update(campaigns_in_stopping_count - campaigns_in_stopping_still_count)
                    time.sleep(5)
    ...
    running_campaigns_after_process = get_campaigns_by_campaign_status(client, "RUNNING")
    ...
    if len(running_campaigns_after_process) == len(originally_running_campaigns):
        ...
        failed_campaigns = list(set(originally_running_campaigns) - set(running_campaigns_after_process))
